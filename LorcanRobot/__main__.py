...
import LorcanRobot.modules.sql.users_sql as sql
from sys import argv
from typing import Optional
from telegram import __version__ as peler
from platform import python_version as memek
from LorcanRobot import (
    ALLOW_EXCL,
    CERT_PATH,
    DONATION_LINK,
    LOGGER,
...
from LorcanRobot.modules import ALL_MODULES
from LorcanRobot.modules.helper_funcs.chat_status import is_user_admin
from LorcanRobot.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("LorcanRobot.modules." + module_name)
        ...
        if len(args) >= 1:
            ...
            update.effective_message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(text=gs(chat.id, "about_button"), callback_data="Lorcan_"),
                        ],
                        [
...
def Lorcan_about_callback(update, context):
    query = update.callback_query
    chat = update.effective_chat
    if query.data == "Lorcan_":
        query.message.edit_text(
    ...
    elif query.data == "Lorcan_support":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(text=gs(chat.id, "support_chat_link_button"), url="t.me/lorcan_x_support"),
                        InlineKeyboardButton(text=gs(chat.id, "updates_channel_link_button"), url="https://t.me/lorcan_x_updates"),
                    ],
                    [
                        InlineKeyboardButton(text=gs(chat.id, "back_button"), callback_data="Lorcan_"),
                    ]
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(
        Lorcan_about_callback, pattern=r"Lorcan_", run_async=True
    )
