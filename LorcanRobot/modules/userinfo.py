...
from LorcanRobot.__main__ import STATS, TOKEN, USER_INFO
from LorcanRobot.modules.sql import SESSION
import LorcanRobot.modules.sql.userinfo_sql as sql
from LorcanRobot.modules.disable import DisableAbleCommandHandler
from LorcanRobot.modules.sql.global_bans_sql import is_user_gbanned
from LorcanRobot.modules.sql.afk_sql import is_afk, set_afk
from LorcanRobot.modules.sql.users_sql import get_user_num_chats
from LorcanRobot.modules.helper_funcs.chat_status import sudo_plus
from LorcanRobot.modules.helper_funcs.extraction import extract_user
...
def info(update: Update, context: CallbackContext):
    ...
    rep = message.reply_text("<code>Gᴇᴛᴛɪɴɢ Iɴғᴏ Oғ Tʜɪs Pᴇʀsᴏɴ..</code>", parse_mode=ParseMode.HTML)
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>╔═━「 Lᴏʀᴄᴀɴ Sᴛᴀᴛɪsᴛɪᴄs 」━═</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)
    result += "\n<b>╘═━「 Pᴏᴡᴇʀᴇᴅ Bʏ Lᴏʀᴄᴀɴ 」━═</b>"
    update.effective_message.reply_text(
        result,
        parse_mode=ParseMode.HTML, 
        disable_web_page_preview=True
